...
export const Layout: React.FC<LayoutProps> = ({ currentPage, onNavigate, children }) => {
  const { user, setIsVoiceListening, setActiveQuickAction, updateUserPreferences } = useApp();
  ...
  const navItems = [
    ...
    { name: 'Painel Admin', page: 'Admin' as PageName, icon: ShieldAlert, badge: 'ROOT' },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#02050e] text-slate-100 font-inter">
      ...
      <aside className="hidden md:flex flex-col w-72 shrink-0 border-r border-cyan-500/20 bg-[#040817]/95 backdrop-blur-xl h-screen sticky top-0 z-40 overflow-y-auto">
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-cyan-500/20 text-cyan-300 font-mono font-bold">PRO</span>
              </div>
              <p className="text-[10px] text-cyan-300/70 font-mono uppercase tracking-widest">Command Center</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-white/5 bg-[#030612]/80">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-cyan-400/80 font-mono truncate">{user.alias || 'Chefe'}</p>
            </div>
            <button
      ...
      <header className="md:hidden flex items-center justify-between px-4 py-3 border-b border-cyan-500/20 bg-[#040817]/95 sticky top-0 z-50 backdrop-blur-lg">
        ...
        >
          ...
          <span className="font-rajdhani text-xl font-bold tracking-wider text-white">JARVES</span>
        </div>
